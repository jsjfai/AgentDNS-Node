...
description: 'Configure MCP servers and their settings for AIAgentDNS·DNSNode'
---

# MCP Settings Configuration

This guide explains how to configure MCP servers in AIAgentDNS·DNSNode using the `mcp_settings.json` file and related configurations.

## Configuration Files Overview

AIAgentDNS·DNSNode uses several configuration files:
...
```json
{
  "mcpServers": {
    "fetch": {
      ...
      "env": {
        "USER_AGENT": "AIAgentDNS·DNSNode/1.0"
      }
    },
    "playwright": {
      "command": "npx",
...
```json
{
  "osm": {
    ...
    "env": {
      "OSM_USER_AGENT": "AIAgentDNS·DNSNode/1.0"
    }
  }
}
```
...
AIAgentDNS·DNSNode supports environment variable substitution using `${VAR_NAME}` syntax:
...
```json
{
  "servers": {
    "fetch": {
      ...
      "tags": ["http", "api", "web"],
      "version": "1.0.0",
      "author": "AIAgentDNS·DNSNode Team",
...
AIAgentDNS·DNSNode supports hot reloading of configurations:
...
AIAgentDNS·DNSNode validates configurations on startup and reload:
...
This comprehensive guide covers all aspects of configuring MCP servers in AIAgentDNS·DNSNode for various use cases and environments.
