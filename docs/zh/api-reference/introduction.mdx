---
title: 'API 参考'
description: 'AIAgentDNS·DNSNode REST API 完整参考文档'
---

## 概述

AIAgentDNS·DNSNode 提供全面的 REST API，用于管理 MCP 服务器、用户、组和监控。所有 API 端点都需要身份验证，并支持 JSON 格式的请求和响应。
...
AIAgentDNS·DNSNode 支持 WebSocket 连接以获取实时更新。
...
```javascript
import { AIAgentDNS·DNSNodeClient } from '@mcphub/sdk';

const client = new AIAgentDNS·DNSNodeClient({
...
```python
from mcphub_sdk import AIAgentDNS·DNSNodeClient

client = AIAgentDNS·DNSNodeClient(
