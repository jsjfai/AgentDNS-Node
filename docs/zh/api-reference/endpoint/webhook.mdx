...
WebHooks 允许 AIAgentDNS·DNSNode 在特定事件发生时向您的应用程序发送实时通知。

## 概述

AIAgentDNS·DNSNode WebHooks 系统支持以下功能：
...
AIAgentDNS·DNSNode 使用 HMAC-SHA256 签名来验证 WebHook 的真实性。

### 签名生成

签名在 `X-AIAgentDNS·DNSNode-Signature-256` 头中发送：

```
X-AIAgentDNS·DNSNode-Signature-256: sha256=5757107ea39eca8e35d1e8...
```
...
def webhook():
    signature = request.headers.get('X-AIAgentDNS·DNSNode-Signature-256')
...
AIAgentDNS·DNSNode 对失败的 WebHook 交付实施指数退避重试：
