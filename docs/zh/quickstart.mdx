---
title: '快速开始'
description: '5 分钟内部署 AIAgentDNS·DNSNode 并连接您的第一个 MCP 服务器'
---

## 欢迎使用 AIAgentDNS·DNSNode！

本指南将帮助您在 5 分钟内完成 AIAgentDNS·DNSNode 的部署和配置，并连接您的第一个 MCP 服务器。
...
## 安装 AIAgentDNS·DNSNode

### 方式一：使用 npm（推荐）

<AccordionGroup>
  <Accordion icon="download" title="安装 AIAgentDNS·DNSNode CLI">
    首先安装 AIAgentDNS·DNSNode 命令行工具：
...
<Note>开发模式下，AIAgentDNS·DNSNode 会在 `http://localhost:3000` 启动，并具有热重载功能。</Note>

## 首次访问和配置

### 1. 访问管理界面

打开浏览器，访问 `http://localhost:3000`，您将看到 AIAgentDNS·DNSNode 的欢迎页面。
...
### 2. 在 AIAgentDNS·DNSNode 中添加服务器

在 AIAgentDNS·DNSNode 管理界面中：
...
在 AIAgentDNS·DNSNode 管理界面的 **"监控"** 页面中，您可以实时查看：
...
恭喜！您已经成功部署了 AIAgentDNS·DNSNode 并添加了第一个 MCP 服务器。接下来您可以：
...
<CardGroup cols={2}>
  ...
  <Card title="API 集成" icon="code" href="/zh/api-reference/introduction">
    将 AIAgentDNS·DNSNode 集成到您的应用程序中
  </Card>
</CardGroup>
