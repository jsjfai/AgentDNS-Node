---
title: 'Nginx 配置'
description: '配置 Nginx 作为 AIAgentDNS·DNSNode 的反向代理'
---

# Nginx 配置

本指南说明如何配置 Nginx 作为 AIAgentDNS·DNSNode 的反向代理，包括 SSL 终止、负载均衡和缓存策略。
...
对于具有多个 AIAgentDNS·DNSNode 实例的高可用性设置：
...
当在 Docker 中运行 AIAgentDNS·DNSNode 时，更新代理配置：
...
**502 Bad Gateway**：检查 AIAgentDNS·DNSNode 是否正在运行且可访问
...
此配置为在 Nginx 后运行 AIAgentDNS·DNSNode 提供了坚实的基础，具有适当的安全性、性能和可靠性功能。
