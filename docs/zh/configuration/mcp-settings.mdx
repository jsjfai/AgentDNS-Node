---
title: 'MCP 设置配置'
description: '配置 AIAgentDNS·DNSNode 的 MCP 服务器及其设置'
---

# MCP 设置配置

本指南说明如何使用 `mcp_settings.json` 文件和相关配置在 AIAgentDNS·DNSNode 中配置 MCP 服务器。

## 配置文件概述

AIAgentDNS·DNSNode 使用几个配置文件：

- **`mcp_settings.json`**：主要的 MCP 服务器配置
- **`servers.json`**：服务器元数据和场景组织
- **`.env`**：环境变量和密钥
...
AIAgentDNS·DNSNode 支持使用 `${VAR_NAME}` 语法进行环境变量替换：
...
AIAgentDNS·DNSNode 在启动和重新加载时验证配置：
...
这个全面的指南涵盖了在 AIAgentDNS·DNSNode 中为各种用例和环境配置 MCP 服务器的所有方面。
