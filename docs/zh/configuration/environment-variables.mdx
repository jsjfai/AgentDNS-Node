---
title: '环境变量配置'
description: '使用环境变量配置 AIAgentDNS·DNSNode'
---

# 环境变量配置

AIAgentDNS·DNSNode 使用环境变量进行配置。本指南涵盖所有可用变量及其用法。
...
AIAgentDNS·DNSNode 按以下顺序加载环境变量：
...
AIAgentDNS·DNSNode 在启动时验证环境变量。无效配置将阻止应用程序启动并提供有用的错误消息。
...
这个全面的环境配置确保 AIAgentDNS·DNSNode 可以为任何部署场景正确配置。
