...
export const useMarketData = () => {
  ...
  // Fetch installed servers (only when authenticated)
  const fetchInstalledServers = useCallback(async (isAuthenticated: boolean) => {
    if (!isAuthenticated) {
      setInstalledServers([]);
      return;
    }
  ...
  const installServer = useCallback(
    async (server: MarketServer, customConfig: ServerConfig) => {
      try {
        ...
        await fetchInstalledServers(!!token);
        return true;
      } catch (err) {
  ...
  // Load installed servers only when auth state changes
  useEffect(() => {
    const token = localStorage.getItem('mcphub_token');
    fetchInstalledServers(!!token);
