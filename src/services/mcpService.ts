      ...
      tools: [
        {
          ...
          description: (() => {
            ...
            const serversList = availableServers
              ...
              .join(', ');
            return `STEP 1 of 2: Use this tool FIRST to discover and search for relevant tools across all available servers. This tool and execute_tool work together as a two-step process: 1) get_available_tools to find what you need, 2) execute_tool to execute it.
...
After finding relevant tools, you MUST use the execute_tool to actually execute them. The get_available_tools only finds tools - it doesn't execute them.
...
Available servers: ${serversList}`;
        ...
        },
        {
          name: 'execute_tool',
          description:
            "STEP 2 of 2: Use this tool AFTER get_available_tools to actually execute/invoke any tool you found. This is the execution step - get_available_tools finds tools, execute_tool runs them.\n\nWorkflow: get_available_tools → examine results → execute_tool with the chosen tool name and required arguments.\n\nIMPORTANT: Always check the tool's inputSchema from get_available_tools results before invoking to ensure you provide the correct arguments. The search results will show you exactly what parameters each tool expects.",
          inputSchema: {
            type: 'object',
            properties: {
              toolName: {
                type: 'string',
                description: 'The exact name of the tool to invoke (from get_available_tools results)',
              },
              arguments: {
                type: 'object',
                description:
...
export const handleCallToolRequest = async (request: any, extra: any) => {
  ...
  try {
    ...
    if (request.params.name === 'get_available_tools') {
      ...
      const response = {
        ...
        metadata: {
          ...
          nextSteps:
            tools.length > 0
              ? 'To use a tool, call execute_tool with the toolName and required arguments.'
    ...
    // Special handling for execute_tool
    if (request.params.name === 'execute_tool') {
