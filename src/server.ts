...
export class AppServer {
  ...
  private findAndServeFrontend(): void {
    ...
    if (this.frontendPath) {
      ...
      this.app.get(rootPath, (_req, res) => {
        res
          .status(404)
          .send('Frontend not found. AIAgentDNS·DNSNode API is running, but the UI is not available.');
      });
    }
  }
  ...
  start(): void {
    this.app.listen(this.port, () => {
      ...
      if (this.frontendPath) {
        console.log(`Open http://localhost:${this.port} in your browser to access AIAgentDNS·DNSNode UI`);
      } else {
        console.log(
          `AIAgentDNS·DNSNode API is running on http://localhost:${this.port}, but the UI is not available`,
        );
      }
    });
  }
    ...
    for (const root of possibleRoots) {
      ...
      if (fs.existsSync(packageJsonPath)) {
        try {
          ...
          if (pkg.name === 'agentweb' || pkg.name === '@doesai/agentweb') {
            if (debug) {
