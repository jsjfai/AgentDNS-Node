# 无限工具，智能路由：AIAgentDNS·DNSNode 引领 AI 工具使用新范式
...
添加成功后，就可以在工具中看到 `get_available_tools` 和 `execute_tool` 两个工具了：
